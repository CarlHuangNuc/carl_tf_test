...
ap.add_argument('--model', type=str , default="/mnt/huangke1/LIM/Qwen2.5-Omni-7B", help="huggingface model name or local path")
args = ap.parse_args()
...
class StreamManager:
    def __init__(self):
        ...
        with torch.no_grad():
            self.minicpmo_model = Qwen2_5OmniForConditionalGeneration.from_pretrained(self.minicpmo_model_path, attn_implementation='sdpa', torch_dtype="auto",  device_map="auto")
        ...
        self.processor = Qwen2_5OmniProcessor.from_pretrained(self.minicpmo_model_path)
            ...
            if (len(self.audio_prefill) == (2000/self.audio_chunk)) or (is_end and len(self.audio_prefill)>0):
                ...
                with torch.no_grad():
                    ...
                    if cnts is not None:
                        ...
                        msgs = [msg]
                        ## change msgs to inputs
                        print("call model ....interface .........")
                        ...
                        ###carl change interface for streaming ...
    ...
    def test_carl(self):
        ...
        with torch.inference_mode():
            ...
            try:
                ...
                text = self.processor.apply_chat_template(conversation, add_generation_prompt=True, tokenize=False)
                audios, images, videos = process_mm_info(conversation, use_audio_in_video=USE_AUDIO_IN_VIDEO)
                print(self.input_audio_id)
                exit()
                print(text)
                print(audios)    
                inputs = self.processor(text=text, audio=audios, images=images, videos=videos, return_tensors="pt", padding=True, use_audio_in_video=USE_AUDIO_IN_VIDEO,stream=True,first_slice=True,end_slice=True)
                print(inputs)
                exit()

                inputs = inputs.to(self.minicpmo_model.device).to(self.minicpmo_model.dtype)
                text_ids = self.minicpmo_model.generate(**inputs,use_audio_in_video=USE_AUDIO_IN_VIDEO, return_audio=False, use_stream=True, is_end=True)
